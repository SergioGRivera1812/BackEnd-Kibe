const bascula1ForaneosModel = require('../models/registroBascula1Foraneos');

const bascula1ForaneosController = {
    getAllRegistrosForaneos: async (req, res) => {
        try {
            const registros = await bascula1ForaneosModel.getAllRegistrosForaneos();
            res.json(registros);
        } catch (error) {
    ...
    getRegistroByIdForaneos: async (req, res) => {
        try {
            const registro = await bascula1ForaneosModel.getRegistroByIdForaneos(req.params.id);
            if (!registro) {
    ...
    getRegistroByIdCamionForaneos: async (req, res) => {
        try {
            ...
            const registro = await bascula1ForaneosModel.getRegistroByIdCamionForaneos(req.params.idCamion);
    ...
    createRegistroForaneos: async (req, res) => {
        try {
            const registro = await bascula1ForaneosModel.createRegistroForaneos(req.body);
    ...
    updateRegistroForaneos: async (req, res) => {
        try {
            ...
            const result = await bascula1ForaneosModel.updateRegistroForaneos(
                idCamion,
                safeValues.bruto,
                safeValues.neto,
                safeValues.fechaS,
    ...
    getTaraByIdCamionForaneos: async (req, res) => {
        try {
            const idCamion = req.params.idCamion;
            const tara = await bascula1ForaneosModel.getTaraByIdCamionForaneos(idCamion);
            res.json({ tara });
        } catch (error) {
    ...
    deleteRegistroForaneos: async (req, res) => {
        try {
            const registro = await bascula1ForaneosModel.deleteRegistro(req.params.id);
    ...
    getRegistroByIdOrUsernameForaneos: async (req, res) => {
        try {
            ...
            if (id) {
                ...
                const registro = await bascula1ForaneosModel.getRegistroByIdForaneos(id);
                if (!registro) {
                    return res.status(404).json({ error: 'Registro no encontrado' });
                }
                ...
                const registro = await bascula1ForaneosModel.getRegistroByUsuarioForaneos(usuario);
                if (!registro) {
    ...
    RegistrarAperturarB1: async (req, res) => {
        try {
            const registro = await bascula1ForaneosModel.RegistrarAperturarB1(req.body);
            res.status(201).json({ message: 'Registro creado', id: registro.insertId });
        } catch (error) {
...
module.exports = bascula1ForaneosController;